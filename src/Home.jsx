...
const Home = () => {
  ...
  const handleStateChange = (e) => {
    ...
    axios
      .get(
        `https://tss11-30-project.onrender.com/api/v1/city/getcity/${selectedState}`
      )
      .then((response) => {
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        await axios
          .get(
            `https://api.geoapify.com/v1/geocode/reverse?lat=${latitude}&lon=${longitude}&apiKey=4c14f9f966c0424fa6f8c75b884836b2`
          )
          .then((response) => {
            ...
            axios
              .get(
                `http://api.weatherapi.com/v1/current.json?key=8b26d241c5564b52a41183325251302&q=${city}&aqi=no`
              )
              .then((response) => {
